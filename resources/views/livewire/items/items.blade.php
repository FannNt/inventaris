            ...
            @foreach($items as $item)
                @php
                    ...
                    $statusConfig = !$expiryDate ? [
                        ...
                        'text' => 'text-red-700',
                        'status' => 'Expired'
                    ] : ($expiryDate->lt($threeMonthsFromNow) ? [
                        'bg' => 'bg-amber-50/50',
                ...
                <div class="group relative bg-white border border-gray-200 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 {{ $statusConfig['bg'] }}">
                    ...
                    <div class="absolute top-4 right-4 z-10">
                        <div class="flex items-center space-x-2">
                            <span class="px-2.5 py-1 rounded-full text-xs font-medium {{ $item->kondisi === 'Baik' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $item->kondisi === 'Baik' ? 'Good' : 'Damaged' }}
                            </span>
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    @else
                                        <p class="font-medium {{ $statusConfig['text'] }}">
                                            {{ \Carbon\Carbon::parse($item->masa_berlaku)->format('M d, Y') }}
                                        </p>
                                    @endif
                                </div>
                            </div>
